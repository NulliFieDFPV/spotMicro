...
from lib import spot_lib
from lib import spot_cg_lib
#from lib import spot_amimate_lib
import matplotlib.pyplot as plt
from digitalio import DigitalInOut
from lib.spot_display import dsp
from lib.spot_ctrl_lib import spot_controller
...
Spot = spot_lib.Spot()
SpotCG = spot_cg_lib.SpotCG()
#SpotAnim = spot_amimate_lib.SpotAnim()
...
but_confirm_shutdown=0
but_trigger_shutdown=0
but_init_shutdown=1
...
pos_turn = 3
pos_rightpaw = 5
pos_leftpaw = 4  # also used for hind leg lifting when peeing
...
zeroangle_x = 0.06424343533256503
zeroangle_y = -0.1009112228329157
# zeroangle_x=0
# zeroangle_y=0
...
def createChans(c, bRefresh=False):
    global anim, move, chans_old, joystick
    ...
    #c += 'Anima: %s\n' % ("On" if anim else "Off")  # +('%.2f' % (chan.voltage*2.0035))+' V'
    c += 'Move: %s\n' % ("On" if move else "Off")  # +('%.2f' % (chan.voltage*2.0035))+' V'
    c += 'Rx F/S: %s\n' % (joystick.FsStatus)
    c += 'Rx Sync: %s\n' % (joystick.InSync)
    c += 'Rx RSSI: %s\n' % (joystick.rssi)
    return c
...
def moving(pos, move):
    ...
    if move == True:
        if (thetalf_reply[1] == False):
            try:
                #print(Spot.angle_scale_factor_lf1, Spot.dir01, Spot.zero01)
                #print(Spot.angle_scale_factor_lf2, Spot.dir02, Spot.zero02)
                #print(Spot.angle_scale_factor_lf3, Spot.dir03, Spot.zero03)
                kit.servo[Spot.servo_table[0]].angle = thetalf[
                                                           0] / pi * 180 * Spot.angle_scale_factor_lf1 * Spot.dir01 + Spot.zero01
                kit.servo[Spot.servo_table[1]].angle = thetalf[
                                                           1] / pi * 180 * Spot.angle_scale_factor_lf2 * Spot.dir02 + Spot.zero02
                kit.servo[Spot.servo_table[2]].angle = thetalf[
                                                           2] / pi * 180 * Spot.angle_scale_factor_lf3 * Spot.dir03 + Spot.zero03
            except ValueError:
                print('Angle out of Range')

        if (thetarf_reply[1] == False):
            try:
                kit.servo[Spot.servo_table[3]].angle = thetarf[
                                                           0] / pi * 180 * Spot.angle_scale_factor_rf1 * Spot.dir04 + Spot.zero04
                kit.servo[Spot.servo_table[4]].angle = thetarf[
                                                           1] / pi * 180 * Spot.angle_scale_factor_rf2 * Spot.dir05 + Spot.zero05
                kit.servo[Spot.servo_table[5]].angle = thetarf[
                                                           2] / pi * 180 * Spot.angle_scale_factor_rf3 * Spot.dir06 + Spot.zero06
            except ValueError:
                print('Angle out of Range')

        if (thetarr_reply[1] == False):
            try:
                kit.servo[Spot.servo_table[6]].angle = thetarr[
                                                           0] / pi * 180 * Spot.angle_scale_factor_rr1 * Spot.dir07 + Spot.zero07
                kit.servo[Spot.servo_table[7]].angle = thetarr[
                                                           1] / pi * 180 * Spot.angle_scale_factor_rr2 * Spot.dir08 + Spot.zero08
                kit.servo[Spot.servo_table[8]].angle = thetarr[
                                                           2] / pi * 180 * Spot.angle_scale_factor_rr3 * Spot.dir09 + Spot.zero09
            except ValueError:
                print('Angle out of Range')

        if (thetalr_reply[1] == False):
            try:
                kit.servo[Spot.servo_table[9]].angle = thetalr[
                                                           0] / pi * 180 * Spot.angle_scale_factor_lr1 * Spot.dir10 + Spot.zero10
                kit.servo[Spot.servo_table[10]].angle = thetalr[
                                                            1] / pi * 180 * Spot.angle_scale_factor_lr2 * Spot.dir11 + Spot.zero11
                kit.servo[Spot.servo_table[11]].angle = thetalr[
                                                            2] / pi * 180 * Spot.angle_scale_factor_lr3 * Spot.dir12 + Spot.zero12
            except ValueError:
                print('Angle out of Range')
...
joystick = spot_controller("/dev/ttyAMA0")
...
pos_init = [-x_offset, track, -b_height, -x_offset, -track, -b_height, -x_offset, -track, -b_height, -x_offset, track,
            -b_height]
...
pos = [-x_offset, track, -b_height, -x_offset, -track, -b_height, -x_offset, -track, -b_height, -x_offset, track,
       -b_height, theta_spot, x_spot, y_spot, z_spot]
...
for i in range(200):
    sleep(0.001)
disptext = 'Ready !\n'
setText(disptext + createChans(chans))
...
def shutdownPi():
    ...
    global continuer, bInMenu

    bInMenu=True

    if joystick.getCustom(but_trigger_shutdown)==0:
        setText("Shutdown\ncanceled")
        bInMenu =False
        return 0
...
while (continuer):
    ...
    if iShutdownState == 0:
        if joystick.getCustom(but_trigger_shutdown)==1:
            setText("To Trigger\nShutdown,\nrelease SH")
            iShutdownState=1
            bInMenu = True

    elif iShutdownState >=3:
        setText("Shutdown")
        iShutdownState= shutdownPi()
        if iShutdownState ==1:
            continuer=False
        else:
            reFreshText(createChans(chans, True))


    else:
        if iShutdownState==1:
            if joystick.getCustom(but_init_shutdown) ==1:
                setText("To Confirm\nShutdown,\npull SH again")
                iShutdownState += 1
            else:
                if joystick.getCustom(but_trigger_shutdown) == 0:
            ...
            elif joystick.getCustom(but_init_shutdown) ==0:
                setText("Shutdown\ncanceled")
                bInMenu = False
                iShutdownState = 0

    for i in range(0, joystick.MAX_AXIS):  # read analog joystick position
        #print(i, joystick.getAxis(i))
        joypos[i] = joystick.getAxis(i)

        # for i in range (0,8): #read analog joystick position
        #    print(i, joystick.get_axis(i))

    if joystick.FsStatus != "Ok" or joystick.InSync == False:
        #print(booRxLost)
        if booRxLost == False:
            disptext = 'RX Error...\n'
            setText(disptext + createChans(chans))
    ...
    for i in range(0, joystick.MAX_BUTTONS-1):  # read buttons
        joybut[i] = joystick.getButton(i)
    ...
    if (joybut[but_walk] == 0) & (joybut[but_sit] == 0) & (joybut[but_pee] == 0) & (joybut[but_lie] == 0) & (
            joybut[but_twist] == 0) & (joybut[but_move] == 0) & (joybut[but_anim] == 0) & (lock == True):
        lock = False
    ...
    if (walking == True):
        ...
        if (t < tstart):
            ...
            if (t < tstop):
                ...
                """ stop """
                phase = 5 + trans
        pos = Spot.start_walk_stop(track, x_offset, steering, walking_direction, cw, walking_speed, v_amp, height,
                                   stepl, t, tstep, theta_spot, x_spot, y_spot, z_spot, phase)
        theta_spot = pos[12]
        x_spot = pos[13]
        y_spot = pos[14]
        z_spot = pos[15]
    ...
    if (sitting == True):
        ...
        if (t == 1):  # pawing is possible
            ...
            joypar_old = joypar
            #print(joypal, joypos[pos_rightpaw], joypar)
    ...
    if (shifting == True):
        ...
        start_frame_pos = [0, 0, 0, x_offset, 0, b_height]  # x,y,z rotations then translations
        end_frame_pos = [0, 0, 0, x_end_shifting + x_offset, y_end_shifting,
                         b_height]  # x,y,z rotations then translations
        pos = Spot.moving(t, start_frame_pos, end_frame_pos, pos)
        ...
        if (t == 1):  # pawing is possible
            ...
            if (peeing == True):
                ...
                y_spot_shift = pos[14]
                z_spot_shift = pos[15]
                M = Spot.xyz_rotation_matrix(theta_spot_shift[3], theta_spot_shift[4],
                                             theta_spot_shift[2] + theta_spot_shift[5], False)
                pee_lr = Spot.new_coordinates(M, xleglr, yleglr, zleglr, x_spot_shift[1], y_spot_shift[1],
                                              z_spot_shift[1])
            ...
            if ((joypos[pos_rightpaw] != 0) & (joypos[pos_rightpaw] != -1)) | (joype != -1):
                peeing = True
                if (joypos[pos_rightpaw] >= joype):
                    joype = min(joypos[pos_rightpaw], joype + 0.1)
                else:
                    joype = max(joypos[pos_rightpaw], joype - 0.1)
            else:
                peeing = False
    ...
    if (twisting == True):
        ...
        if (t < 0.25):
            end_frame_pos = [x_angle_twisting, y_angle_twisting, z_angle_twisting, x_offset, 0,
                             b_height]  # x,y,z rotations then translations
    ...
    if (pos[15][5] < 10.01):
        ...
        # SpotAnim.animate(pos, t, pi / 12, -135 / 180 * pi, Angle, center_x, center_y, thetalf, thetarf, thetarr,
        #                 thetalr, walking_speed, walking_direction, steering, stance)
    ...
    if (datetime.now() - tftDisp.lastUpdate).total_seconds()>2 and bInMenu == False and Free:
        reFreshText(createChans(chans, True))

    Bat = Bat + 1
    if (Bat == 30):  # update Battery voltage
        # chan = AnalogIn(ads, ADS.P0)
        chans = createChans(chans, True)
