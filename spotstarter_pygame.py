...
def createChans(c, bRefresh=False):
    ...
    c += 'Move: %s' % ("On" if move else "Off")  # +('%.2f' % (chan.voltage*2.0035))+' V'
...
def moving(pos, move):
    ...
    if move == True:
        if (thetalf_reply[1] == False):
            try:
                # print(thetalf[0]/pi*180*Spot.angle_scale_factor_lf1*Spot.dir01+Spot.zero01)
                # print(thetalf[1]/pi*180*Spot.angle_scale_factor_lf2*Spot.dir02+Spot.zero02)
                # print(thetalf[2]/pi*180*Spot.angle_scale_factor_lf3*Spot.dir03+Spot.zero03)
