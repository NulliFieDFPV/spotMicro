...
class SpotAnim:
    ...
    def animate(self, pos, t, thetax, thetaz, angle, center_x, center_y, thetalf, thetarf, thetarr, thetalr,
                walking_speed, walking_direction, steering, stance):
        ...
        line = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0],
                                       [theta_spot[0], theta_spot[1], 0, 0, 0, 0], thetax, thetaz, SpotAnim.xFloor,
                                       SpotAnim.yFloor, SpotAnim.zFloor)
        pygame.draw.polygon(screen, SpotAnim.GREY, line, 0)

        """Floor Grid Display """
        for i in range(0, 11):
            line = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0],
                                           [theta_spot[0], theta_spot[1], 0, 0, 0, 0], thetax, thetaz,
                                           [-500 + i * 100, -500 + i * 100], [-500, 500], [0, 0])
            pygame.draw.lines(screen, SpotAnim.DARK_GREY, False, line, 1)
            line = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0],
                                           [theta_spot[0], theta_spot[1], 0, 0, 0, 0], thetax, thetaz, [-500, 500],
                                           [-500 + i * 100, -500 + i * 100], [0, 0])
            pygame.draw.lines(screen, SpotAnim.DARK_GREY, False, line, 1)

        """ X,Y,Z frame display"""
        line = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [0, 0, 0, 0, 0, 0], thetax, thetaz,
                                       SpotAnim.xX, SpotAnim.yX, SpotAnim.zX)
        pygame.draw.lines(screen, SpotAnim.RED, False, line, 2)

        line = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [0, 0, 0, 0, 0, 0], thetax, thetaz,
                                       SpotAnim.xY, SpotAnim.yY, SpotAnim.zY)
        pygame.draw.lines(screen, SpotAnim.GREEN, False, line, 2)

        line = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [0, 0, 0, 0, 0, 0], thetax, thetaz,
                                       SpotAnim.xZ, SpotAnim.yZ, SpotAnim.zZ)
        pygame.draw.lines(screen, SpotAnim.BLUE, False, line, 2)

        """ Radius display """
        center_display = True
        if (steering < 2000):
            lineR = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [0, 0, 0, 0, 0, 0], thetax,
                                            thetaz, [center_x, x_spot[0]], [center_y, y_spot[0]], [0, 0])
        else:
            center_x1 = x_spot[0] + (center_x - x_spot[0]) / steering * 2000
            center_y1 = y_spot[0] + (center_y - y_spot[0]) / steering * 2000
            lineR = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [0, 0, 0, 0, 0, 0], thetax,
                                            thetaz, [center_x1, x_spot[0]], [center_y1, y_spot[0]], [0, 0])
        ...
        lineD = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [0, 0, 0, 0, 0, 0], thetax, thetaz,
        ...
        lineCG = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [0, 0, 0, 0, 0, 0], thetax, thetaz,
        ...
        linelf = SpotAnim.display_rotate(self, x_spot[1] - x_spot[0], y_spot[1] - y_spot[0], z_spot[1] - z_spot[0],
                                         theta_spot, thetax, thetaz, xleglf, yleglf, zleglf)
        ...
        linerf = SpotAnim.display_rotate(self, x_spot[1] - x_spot[0], y_spot[1] - y_spot[0], z_spot[1] - z_spot[0],
                                         theta_spot, thetax, thetaz, xlegrf, ylegrf, zlegrf)
        ...
        linerr = SpotAnim.display_rotate(self, x_spot[1] - x_spot[0], y_spot[1] - y_spot[0], z_spot[1] - z_spot[0],
                                         theta_spot, thetax, thetaz, xlegrr, ylegrr, zlegrr)
        ...
        linelr = SpotAnim.display_rotate(self, x_spot[1] - x_spot[0], y_spot[1] - y_spot[0], z_spot[1] - z_spot[0],
                                         theta_spot, thetax, thetaz, xleglr, yleglr, zleglr)
        """ Body frame lines """
        lineb = SpotAnim.display_rotate(self, x_spot[1] - x_spot[0], y_spot[1] - y_spot[0], z_spot[1] - z_spot[0],
                                        theta_spot, thetax, thetaz, [Spot.xlf, Spot.xrf, Spot.xrr, Spot.xlr, Spot.xlf],
                                        [Spot.ylf, Spot.yrf, Spot.yrr, Spot.ylr, Spot.ylf],
        ...
        linedCG = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [0, 0, 0, 0, 0, 0], thetax, thetaz,
                                          [CGabs[0], dCG[0]], [CGabs[1], dCG[1]], [0, 0])
        lineZMP = SpotAnim.display_rotate(self, -x_spot[0], -y_spot[0], -z_spot[0], [0, 0, 0, 0, 0, 0], thetax, thetaz,
                                          [ZMPabs[0], CGabs[0]], [ZMPabs[1], CGabs[1]], [0, 0])
        if sum(stance) > 2:
